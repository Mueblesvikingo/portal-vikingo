...
function SourceDistributionPie({ items, weeklyPlanKpi }) {
  const total = safeArray(items).reduce((sum, item) => sum + Number(item.monthly || 0), 0);
  const utilization = MONTHLY_CAPACITY_HOURS > 0 ? (total / MONTHLY_CAPACITY_HOURS) * 100 : 0;
...
export default function WorkloadBalanceModule({
  ...
  const peopleOptions = useMemo(
    () => safeArray(peopleCatalog)
      ...
      .map((person) => ({ id: String(getPersonId(person)), name: getPersonName(person) }))
  ...
  const agendaWeekSummary = useMemo(() => WEEK_DAYS.map((day) => { const activities = activeAgendaBlocks.filter((activity) => activity.diaTipico === day); const usedMinutes = activities.reduce((sum, activity) => sum + getDurationMinutes(activity), 0); const occupation = (usedMinutes / DAILY_CAPACITY_MINUTES) * 100; return { day, activities, usedMinutes, occupation, status: getWorkloadStatus(occupation) }; }), [activeAgendaBlocks]);
  const agendaMonthSummary = useMemo(() => [1, 2, 3, 4].map((weekNumber) => { const usedMinutes = activeAgendaBlocks.reduce((sum, activity) => sum + getDurationMinutes(activity), 0); const occupation = (usedMinutes / 60 / WEEKLY_CAPACITY_HOURS) * 100; return { weekNumber, label: `Semana ${weekNumber}`, usedMinutes, occupation, status: getWorkloadStatus(occupation) }; }), [activeAgendaBlocks]);
...
{viewMode === "capacity" && <div className="p-3"><SourceDistributionPie items={sourceSummary} weeklyPlanKpi={allWeeksPlanKpi} /></div>}
            ...
            {viewMode === "pending" && <PendingActivitiesView hasSelectedPerson={effectivePersonFilter !== "all"} activities={filteredPendingActivities} totalHours={pendingTotalHours} onOpenSchedule={openScheduleModal} />}
