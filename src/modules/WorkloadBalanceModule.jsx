...
function PendingActivitiesView({ hasSelectedPerson, activities, totalHours, canEditActivities, onOpenSchedule, onEditActivity }) {
  ...
  const sortedActivities = filteredActivities
    ...
    .sort((a, b) => {
      ...
      if (subprocessFilter !== "all") {
        const aOrder = getPendingActivityOperationalOrder(a.activity);
        const bOrder = getPendingActivityOperationalOrder(b.activity);
...
export default function WorkloadBalanceModule({
  ...
  const agendaMonthSummary = useMemo(() => [1, 2, 3, 4].map((weekNumber) => { const monthlySpecificMinutes = agendaMonthlyMatrix.reduce((sum, row) => { const week = row.weeks.find((item) => item.weekNumber === weekNumber); return sum + Number(week?.usedMinutes || 0); }, 0); const usedMinutes = weeklyTypicalLoad.usedMinutes + monthlySpecificMinutes; const weeklyCapacityHours = selectedPersonCapacity.weeklyHours || WEEKLY_CAPACITY_HOURS; const occupation = weeklyCapacityHours > 0 ? (usedMinutes / 60 / weeklyCapacityHours) * 100 : 0; return { weekNumber, label: `Semana ${weekNumber}`, usedMinutes, monthlySpecificMinutes, weeklyTypicalLoad, occupation, capacityHours: weeklyCapacityHours, status: getWorkloadStatus(occupation) }; }), [agendaMonthlyMatrix, selectedPersonCapacity.weeklyHours, weeklyTypicalLoad]);
          ...
          {agendaMonthSummary.map((week) => (
            <div key={week.weekNumber} className="border-l border-slate-100 px-3 py-2.5">
              ...
              <div className="mt-1 flex items-center justify-between text-[9px] font-black normal-case tracking-normal text-slate-500">
                ...
                <span>{formatHours(week.usedMinutes / 60)} / {formatHours(week.capacityHours)}</span>
              </div>
