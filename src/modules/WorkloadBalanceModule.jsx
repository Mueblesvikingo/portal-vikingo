...
function AssignmentEditModal({ assignment, draft, setDraft, onSave, onClose, puestoOptions = [] }) {
  if (!assignment) return null;

  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4"><div className="w-full max-w-2xl rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl"><div className="mb-3 flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-widest text-slate-900">Editar asignación</p><p className="text-[10px] font-bold text-slate-400">Actualiza los datos básicos de la asignación.</p></div><button type="button" onClick={onClose} className="flex h-7 w-7 items-center justify-center rounded-full border border-slate-200 text-sm font-black text-slate-400 hover:bg-slate-50">×</button></div><div className="grid gap-2 md:grid-cols-2"><label className="md:col-span-2 text-[10px] font-black uppercase tracking-widest text-slate-400">Título<input value={draft.titulo} onChange={(event) => setDraft((current) => ({ ...current, titulo: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-bold normal-case tracking-normal text-slate-700 outline-none" /></label><label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Tipo<select value={draft.tipo} onChange={(event) => setDraft((current) => ({ ...current, tipo: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-bold normal-case tracking-normal text-slate-700 outline-none">{ASSIGNMENT_TYPES.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Prioridad<select value={draft.prioridad} onChange={(event) => setDraft((current) => ({ ...current, prioridad: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-bold normal-case tracking-normal text-slate-700 outline-none">{ASSIGNMENT_PRIORITIES.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Gestionar en<select value={draft.gestionarEn} onChange={(event) => setDraft((current) => ({ ...current, gestionarEn: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-bold normal-case tracking-normal text-slate-700 outline-none">{ASSIGNMENT_CHANNELS.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Carga estimada (horas)<input type="number" min="0.25" step="0.25" value={draft.horas} onChange={(event) => setDraft((current) => ({ ...current, horas: Number(event.target.value) }))} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-bold normal-case tracking-normal text-slate-700 outline-none" /></label><label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Fecha límite<input type="date" value={draft.fechaLimite} onChange={(event) => setDraft((current) => ({ ...current, fechaLimite: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-bold normal-case tracking-normal text-slate-700 outline-none" /></label><label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Revisará<select value={draft.revisara} onChange={(event) => setDraft((current) => ({ ...current, revisara: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-bold normal-case tracking-normal text-slate-700 outline-none"><option value="">Seleccionar puesto</option>{puestoOptions.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Aprobará<select value={draft.aprobara} onChange={(event) => setDraft((current) => ({ ...current, aprobara: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-bold normal-case tracking-normal text-slate-700 outline-none"><option value="">Seleccionar puesto</option>{puestoOptions.map((item) => <option key={item} value={item}>{item}</option>)}<option value="No requiere aprobación">No requiere aprobación</option></select></label><label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Dará seguimiento<select value={draft.seguimiento} onChange={(event) => setDraft((current) => ({ ...current, seguimiento: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-bold normal-case tracking-normal text-slate-700 outline-none"><option value="PMO">PMO</option>{puestoOptions.map((item) => <option key={item} value={item}>{item}</option>)}<option value="Ninguno">Ninguno</option></select></label></div><div className="mt-3 flex items-center justify-end gap-2 rounded-xl border border-slate-100 bg-slate-50 p-2"><button type="button" onClick={onSave} className="rounded-lg bg-[#001225] px-3 py-1.5 text-[10px] font-black text-white">Guardar cambios</button><button type="button" onClick={onClose} className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-[10px] font-black text-slate-500">Cancelar</button></div></div></div>;
}
...
export default function WorkloadBalanceModule({
  ...
  const [assignmentDraft, setAssignmentDraft] = useState({ titulo: "", tipo: "Proyecto", prioridad: "Media", responsable: "", revisara: "", aprobara: "", seguimiento: "PMO", gestionarEn: "Planner", horas: 4, fechaLimite: currentWorkWeek.end });
  const [showMeetingModal, setShowMeetingModal] = useState(false);
  const [meetingDraft, setMeetingDraft] = useState({ titulo: "", participantIds: [], prioridad: "Media", gestionarEn: "Planner", horas: 1, fechaLimite: currentWorkWeek.end });
  ...
  const [assignmentEditDraft, setAssignmentEditDraft] = useState({ titulo: "", tipo: "Proyecto", prioridad: "Media", revisara: "Líder de proceso", aprobara: "Director General", seguimiento: "PMO", gestionarEn: "Planner", horas: 4, fechaLimite: currentWorkWeek.end });
...
function canCreateAssignments() {
  ...
  async function saveEditScheduledBlock() {
    ...
    await loadWorkloadData();
    setScheduleMessage("Bloque manual actualizado correctamente.");
  }
  ...
  async function createMeeting() {
    ...
    setMeetingDraft({ titulo: "", participantIds: [], prioridad: "Media", gestionarEn: "Planner", horas: 1, fechaLimite: currentWorkWeek.end });
    await loadWorkloadData();
  ...
  function openAssignmentEditModal(assignment) {
    ...
    setAssignmentEditDraft({
      ...
      horas: Number(assignment.horas || 4),
  ...
  async function saveAssignmentEdit() {
    ...
    const title = cleanText(assignmentEditDraft.titulo) || `${assignmentEditDraft.tipo} · ${assignmentEditDraft.gestionarEn}`;
    ...
    if (!result?.ok) {
      console.error(result?.error);
      setScheduleMessage("No se pudo actualizar la asignación.");
      return;
    }
