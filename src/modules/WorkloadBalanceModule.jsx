...
export default function WorkloadBalanceModule({
  ...
  // Lista real y completa de roles institucionales (de persona_roles, no un
  // catálogo fijo en código) para el selector "Responsable" de asignaciones.
  const allActiveRoleOptions = useMemo(
    () => [...new Set(activePersonRoleLinks.map(getPersonRoleName).filter(Boolean))].sort((a, b) => a.localeCompare(b)),
    [activePersonRoleLinks]
  );
  useEffect(() => {
