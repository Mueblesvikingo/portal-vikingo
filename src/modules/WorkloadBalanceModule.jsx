                    ...
                    {week.blocks.length > 0 ? week.blocks.map((block, blockIndex) => {
                      ...
                      return (
                        <React.Fragment key={block.monthlyOccurrenceId || block.id}>
                          ...
                          >
                            <SemaforoDot value={block.semaforo} onChange={(next) => handleSemaforoChange(block.sourceActivityId || block.id, next)} disabled={!canEditSelectedPerson} className="absolute left-1 top-1 z-20" />
