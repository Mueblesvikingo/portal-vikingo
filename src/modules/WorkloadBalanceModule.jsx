...
export default function WorkloadBalanceModule({
  ...
  const agendaMonthSummary = useMemo(() => [1, 2, 3, 4].map((weekNumber) => { const usedMinutes = agendaMonthlyMatrix.reduce((sum, row) => { const week = row.weeks.find((item) => item.weekNumber === weekNumber); return sum + Number(week?.usedMinutes || 0); }, 0); const weeklyCapacityHours = selectedPersonCapacity.weeklyHours || WEEKLY_CAPACITY_HOURS; const occupation = weeklyCapacityHours > 0 ? (usedMinutes / 60 / weeklyCapacityHours) * 100 : 0; return { weekNumber, label: `Semana ${weekNumber}`, usedMinutes, occupation, capacityHours: weeklyCapacityHours, status: getWorkloadStatus(occupation) }; }), [agendaMonthlyMatrix, selectedPersonCapacity.weeklyHours]);
