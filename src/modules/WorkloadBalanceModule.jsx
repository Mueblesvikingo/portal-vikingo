...
import { hasWorkloadFullAccess, canEditWorkloadPendingActivities, canEditWorkloadForPersonRoles, isStrategicTeamMember, isDirectorGeneral, canEditPmoProyecto } from "../services/permissionsService";
import { updateActivitySemaforo, getSubprocesosCatalog, updateSubprocesoPrioridad } from "../services/organizationalDesignService";
...
const ASSIGNMENT_STRATEGIC_ORIGINS = ["Estrategia", "Balance de carga", "Operación", "Auditoría", "Indicadores", "Riesgo / Oportunidad", "Mejora continua", "Dirección"];
const ASSIGNMENT_RISK_ORIGIN = "Riesgo / Oportunidad";
...
function ViewTab({ active, children, onClick }) { return <button type="button" onClick={onClick} className={`shrink-0 whitespace-nowrap rounded-lg px-3 py-1 text-[10px] font-black uppercase tracking-widest transition ${active ? "bg-[#001225] text-white shadow-sm" : "bg-white text-slate-500 hover:bg-slate-50"}`}>{children}</button>; }
...
// Formulario rápido de "+ Bloque": en celular, un campo de texto dentro de
// una columna angosta hace que el teclado tape toda la pantalla al enfocarlo.
// En vez de eso, se abre como una hoja fija a la mitad inferior de la
// pantalla (con fondo oscurecido detrás, que cierra el formulario al tocarlo)
// — el teclado empuja esta hoja, no el layout completo del tablero.
function BottomHalfSheet({ open, onClose, title, children }) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/40" onClick={onClose}>
      <div onClick={(event) => event.stopPropagation()} className="max-h-[50vh] w-full overflow-y-auto rounded-t-2xl border-t border-slate-200 bg-white p-3 shadow-2xl sm:max-w-sm sm:rounded-2xl sm:pb-3">
        <div className="mb-2 flex items-center justify-between gap-2">
          <p className="text-[11px] font-black uppercase tracking-widest text-slate-700">{title}</p>
          <button type="button" onClick={onClose} className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-slate-200 text-sm font-black text-slate-400 hover:bg-slate-50">×</button>
        </div>
        {children}
      </div>
    </div>
...
function getViewGuideText(viewMode) {
  const guides = { capacity: "Vista consolidada de distribución de carga por origen. Identifica dónde se concentra el esfuerzo operativo, estratégico y de apoyo.", week: "Semana típica de trabajo. Arrastra actividades entre días, reorganiza prioridades y ajusta proyectos o formación según la capacidad disponible.", month: "Mes típico basado en cuatro semanas de capacidad. Visualiza carga mensual por procesos, proyectos y formación, incluyendo bloques recurrentes.", assignments: "Centro de asignaciones. Registra encargos autorizados y déjalos disponibles para programarse en una semana específica.", pending: "Bandeja de actividades activas asociadas a la persona seleccionada que aún no están programadas en Semana o Mes típico.", agenda: "Programación semanal y mensual. Los líderes planifican el viernes, Dirección da VOBO el lunes y durante la semana se palomean los avances.", projects: "Tablero Gerencial de Proyectos. Visión consolidada del portafolio activo para Dirección y la PMO — no sustituye la gestión operativa detallada de cada proyecto." };
  return guides[viewMode] || "Consulta y ajusta la carga de trabajo según la capacidad disponible.";
}
function PendingActivitiesView({ hasSelectedPerson, activities, excludedActivities, totalHours, canEditActivities, onOpenSchedule, onEditActivity, onRestoreExcludedActivity, subprocesosCatalog, canEditPriority, onUpdateSubprocessPriority }) {
  const [processFilter, setProcessFilter] = useState("all");
  const [subprocessFilter, setSubprocessFilter] = useState("all");
  const [priorityDrafts, setPriorityDrafts] = useState({});
  const subprocesoNombrePorCodigo = useMemo(() => {
    const map = {};
    safeArray(subprocesosCatalog).forEach((sp) => {
      if (sp?.codigo && sp?.nombre) map[sp.codigo] = sp.nombre;
    });
    return map;
  }, [subprocesosCatalog]);
  // Prioridad de carga a Semana Típica: un número por subproceso (lo define
  // Dirección General) para dar visibilidad de a cuál conviene programar
  // primero — no es un dato por actividad, así que se resuelve por código.
  const subprocesoPrioridadPorCodigo = useMemo(() => {
    const map = {};
    safeArray(subprocesosCatalog).forEach((sp) => {
      if (sp?.codigo) map[sp.codigo] = { id: sp.id, prioridad: sp.prioridad_carga ?? null };
    });
    return map;
  }, [subprocesosCatalog]);
  function getSubprocessPriorityValue(subprocessCode) {
    if (!subprocessCode) return null;
    return subprocesoPrioridadPorCodigo[subprocessCode]?.prioridad ?? null;
  }
  function commitPriority(subprocessCode) {
    const entry = subprocesoPrioridadPorCodigo[subprocessCode];
    if (!entry?.id) return;
    const draft = priorityDrafts[subprocessCode];
    if (draft === undefined) return;
    const parsed = draft === "" ? null : Number(draft);
    if (draft !== "" && !Number.isFinite(parsed)) return;
    if (parsed === (entry.prioridad ?? null)) return;
    onUpdateSubprocessPriority?.(entry.id, parsed);
  }
  ...
  const sortedActivities = filteredActivities
    ...
    .sort((a, b) => {
      ...
      const aPriority = getSubprocessPriorityValue(getPendingActivitySubprocess(a.activity));
      const bPriority = getSubprocessPriorityValue(getPendingActivitySubprocess(b.activity));
  ...
  const renderRows = (section) => section.activities.map((activity) => {
    ...
    const priorityValue = getSubprocessPriorityValue(subprocess);
    const priorityDraft = priorityDrafts[subprocess];
    return <tr key={`${section.key}-${activity.id || getPendingActivityKey(activity)}`} className={`align-top hover:bg-slate-50/70 ${inactive ? "bg-gray-50 text-gray-400" : ""}`}><td className="px-2 py-2 text-center">{canEditPriority ? <input type="number" disabled={!subprocess} title={subprocess ? `Prioridad de carga del subproceso ${subprocess}` : "Actividad sin subproceso"} value={priorityDraft !== undefined ? priorityDraft : (priorityValue ?? "")} onChange={(event) => setPriorityDrafts((prev) => ({ ...prev, [subprocess]: event.target.value }))} onBlur={() => commitPriority(subprocess)} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} className="w-12 rounded-lg border border-slate-200 bg-white px-1.5 py-1 text-center text-[10px] font-black text-slate-700 outline-none focus:border-slate-400 disabled:bg-slate-50 disabled:text-slate-300" /> : <span className={`inline-flex h-5 min-w-[20px] items-center justify-center rounded-full border px-1.5 text-[9px] font-black ${priorityValue !== null ? "border-amber-200 bg-amber-50 text-amber-700" : "border-slate-200 bg-slate-50 text-slate-300"}`}>{priorityValue ?? "—"}</span>}</td><td className={`px-3 py-2 font-black leading-tight whitespace-normal ${inactive ? "text-gray-400" : "text-slate-800"}`}>{activity.actividad}</td><td className={`px-3 py-2 font-bold leading-tight whitespace-normal ${inactive ? "text-gray-400" : "text-slate-600"}`}><span className="block">{activity.proceso}</span>{subprocess && <span className={`mt-0.5 block text-[9px] font-bold ${inactive ? "text-gray-400" : "text-slate-400"}`}>{subprocess}{activityNumber ? ` · Act. ${activityNumber}` : ""}</span>}</td><td className={`px-3 py-2 font-bold leading-tight whitespace-normal ${inactive ? "text-gray-400" : "text-slate-600"}`}>{activity.rol}</td><td className={`px-3 py-2 font-bold ${inactive ? "text-gray-400" : "text-slate-500"}`}><span className="block">{translateFrequency(activity.frecuencia)}</span>{section.key === "radar" && <span className={`mt-0.5 block text-[8px] font-black ${inactive ? "text-gray-400" : "text-slate-400"}`}>Mes previsto: {getPendingActivityPlannedMonthLabel(activity)}</span>}</td><td className={`px-3 py-2 font-black ${inactive ? "text-gray-400" : "text-slate-600"}`}>{activity.duracionMinutos} min</td><td className="px-3 py-2"><span className={`rounded-full border px-2 py-0.5 text-[9px] font-black ${inactive ? "border-gray-200 bg-gray-100 text-gray-500" : "border-emerald-100 bg-emerald-50 text-emerald-700"}`}>{translateStatus(statusValue)}</span></td><td className="px-3 py-2 text-right"><div className="flex justify-end gap-1">{canEditActivities && <button type="button" onClick={() => onEditActivity(activity)} className="rounded-lg border border-slate-200 bg-white px-2 py-1 text-[9px] font-black text-slate-500 shadow-sm hover:bg-slate-50">Editar</button>}<button type="button" onClick={() => onOpenSchedule(activity)} className="rounded-lg bg-[#001225] px-2 py-1 text-[9px] font-black text-white shadow-sm hover:bg-slate-800">{section.key === "radar" ? "Mes previsto" : "Programar"}</button></div></td></tr>;
  });

  return <><div className="p-3"><div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 bg-slate-50 px-4 py-3"><div><p className="text-xs font-black uppercase tracking-widest text-slate-800">Pendientes de programación</p><p className="text-[10px] font-bold text-slate-400">Actividades de los roles asignados a la persona que aún no están en Semana o Mes típico.</p></div><div className="flex items-center gap-2"><span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-[10px] font-black text-slate-500">{sortedActivities.length} actividades | {sectionCounts.programmable || 0} programables | {sectionCounts.radar || 0} en radar | {sectionCounts.eventual || 0} eventuales</span>{hasSelectedPerson && <button type="button" onClick={() => setShowExcludedReport(true)} title="Actividades que este usuario quitó de sus pendientes" className="rounded-full border border-slate-200 bg-white px-3 py-1 text-[10px] font-bold text-slate-400 transition hover:border-slate-300 hover:text-slate-600">Quitadas{excludedList.length > 0 ? ` (${excludedList.length})` : ""}</button>}</div></div>{hasSelectedPerson && <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 bg-white px-4 py-2"><select value={processFilter} onChange={(event) => setProcessFilter(event.target.value)} className="h-8 min-w-[220px] rounded-xl border border-slate-200 bg-slate-50 px-3 text-[10px] font-black text-slate-600 outline-none"><option value="all">Todos los procesos</option>{processOptions.map((process) => <option key={process} value={process}>{process}</option>)}</select><select value={subprocessFilter} onChange={(event) => setSubprocessFilter(event.target.value)} disabled={subprocessOptions.length === 0} className="h-8 min-w-[220px] rounded-xl border border-slate-200 bg-slate-50 px-3 text-[10px] font-black text-slate-600 outline-none disabled:text-slate-300"><option value="all">Todos los subprocesos</option>{subprocessOptions.map((subprocess) => <option key={subprocess} value={subprocess}>{subprocess}</option>)}</select><button type="button" onClick={() => { setProcessFilter("all"); setSubprocessFilter("all"); }} className="h-8 rounded-xl border border-slate-200 bg-white px-3 text-[10px] font-black text-slate-400 transition hover:bg-slate-50 hover:text-slate-700">Limpiar filtros</button>{subprocessFilter !== "all" && subprocesoNombrePorCodigo[subprocessFilter] && <span className="min-w-0 flex-1 truncate text-[10px] font-bold italic text-slate-400" title={subprocesoNombrePorCodigo[subprocessFilter]}>— {subprocesoNombrePorCodigo[subprocessFilter]}</span>}</div>}{!hasSelectedPerson ? <div className="px-5 py-10 text-center text-sm font-bold text-slate-400">Selecciona una persona para ver sus actividades pendientes.</div> : sortedActivities.length === 0 ? <div className="px-5 py-10 text-center text-sm font-bold text-slate-400">No hay actividades pendientes por programar.</div> : <div className="divide-y divide-slate-100">{visibleSections.map((section) => <section key={section.key}><div className="flex items-center justify-between gap-2 bg-white px-4 py-2"><div><p className="text-[10px] font-black uppercase tracking-widest text-slate-700">{section.title} ({section.activities.length})</p><p className="text-[9px] font-bold text-slate-400">{section.description}</p></div><span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-[9px] font-black text-slate-400">{section.activities.length} {section.countLabel}</span></div><div className="overflow-x-auto"><table className="min-w-full table-fixed divide-y divide-slate-100 text-[10px]"><thead className="bg-slate-50 text-left font-black uppercase tracking-[0.14em] text-slate-400"><tr><th className="w-[6%] px-2 py-2 text-center" title="Prioridad de carga a Semana Típica, definida por Dirección General">Prior.</th><th className="w-[27%] px-3 py-2">Actividad</th><th className="w-[16%] px-3 py-2">Proceso</th><th className="w-[16%] px-3 py-2">Rol</th><th className="w-[9%] px-3 py-2">Frecuencia</th><th className="w-[8%] px-3 py-2">Duración</th><th className="w-[8%] px-3 py-2">Estado</th><th className="w-[10%] px-3 py-2 text-right">Acción</th></tr></thead><tbody className="divide-y divide-slate-100">{renderRows(section)}</tbody></table></div></section>)}</div>}</div></div>{showExcludedReport && <ExcludedActivitiesReportModal activities={excludedList} onClose={() => setShowExcludedReport(false)} onRestore={onRestoreExcludedActivity} />}</>;
}
...
export default function WorkloadBalanceModule({
  ...
  const [subprocesosCatalog, setSubprocesosCatalog] = useState([]);

  async function loadWorkloadData() {
    setLoadingActivities(true);

    const [data, peopleData, personRolesData, weeklyPlansData, monthlyPlansData, assignmentsData, liderProcesoOverridesData, subprocesosData] = await Promise.all([
    ...
    setSubprocesosCatalog(safeArray(subprocesosData));
  ...
  async function loadPmoData() {
    ...
    setPmoRecordatorios(recordatorios);
    setPmoLoading(false);
  }
  useEffect(() => {
    loadPmoData();
  ...
  async function handleUpdatePmoProyecto(proyecto, changes) {
    ...
    const result = await updateProyecto(proyecto.id, changes, { actor: currentUser, previous: proyecto });
    if (!result.ok) { console.error(result.error); setPmoMessage("No fue posible guardar el cambio del proyecto."); return; }
    setPmoProyectos((current) => current.map((item) => (item.id === proyecto.id ? result.data : item)));
  }

  async function handleCreatePmoRecordatorio(proyecto) {
    if (!isStrategicTeamMember(currentUser) || !proyecto.lider_proyecto_persona_id) return;
    const mensaje = pmoRecordatorioText.trim();
    if (!mensaje) return;
    const result = await createRecordatorio(
      { proyectoId: proyecto.id, destinatarioPersonaId: proyecto.lider_proyecto_persona_id, mensaje },
      { actor: currentUser }
    );
  ...
  const [pmoProyectos, setPmoProyectos] = useState([]);
  const [pmoRecordatorios, setPmoRecordatorios] = useState([]);
  ...
  const [assignmentDraft, setAssignmentDraft] = useState({ nombre: "", descripcion: "", origenEstrategico: "Balance de carga", titulo: "", tipo: "Proyecto", prioridad: "Media", personaId: "", responsable: "", revisara: "", aprobara: "", seguimiento: "PMO", gestionarEn: "Planner", horas: 4, fechaInicio: "", fechaLimite: currentWorkWeek.end, urlExterna: "" });
  ...
  // monthlyTypicoMatrix ya incluye la carga base de Semana típica
  // consolidada (ver definición arriba), así que el total de la semana es
  // directo -- sumarle weeklyTypicalLoad.usedMinutes aparte lo duplicaría.
  const typicalMonth = useMemo(() => [1, 2, 3, 4].map((weekNumber) => { const weekTotalMinutes = monthlyTypicoMatrix.reduce((sum, row) => { const week = row.weeks.find((item) => item.weekNumber === weekNumber); return sum + Number(week?.usedMinutes || 0); }, 0); const totalSemana = weekTotalMinutes / 60; const weeklyCapacityHours = selectedPersonCapacity.weeklyHours; const occupation = weeklyCapacityHours > 0 ? (totalSemana / weeklyCapacityHours) * 100 : 0; return { week: `Semana ${weekNumber}`, total: totalSemana, totalSemana, monthlySpecificMinutes: weekTotalMinutes, occupation, status: getWorkloadStatus(occupation), capacityHours: weeklyCapacityHours }; }), [monthlyTypicoMatrix, selectedPersonCapacity]);
...
function canCreatePersonScopedBlock() {
  ...
  function renderQuickBlockControl(dayName, hasTopMargin = false) {
    return <>
      <AddBlockButton onClick={() => openQuickProjectForm(dayName)} className={hasTopMargin ? "mt-1" : ""} />
      <BottomHalfSheet open={quickProjectDay === dayName} onClose={cancelQuickProject} title={`+ Bloque · ${dayName}`}>
        <QuickBlockForm day={dayName} editingProjectId={editingProjectId} quickProjectType={quickProjectType} setQuickProjectType={setQuickProjectType} quickProjectName={quickProjectName} setQuickProjectName={setQuickProjectName} quickProjectMinutes={quickProjectMinutes} setQuickProjectMinutes={setQuickProjectMinutes} saveQuickProject={saveQuickProject} cancelQuickProject={cancelQuickProject} />
      </BottomHalfSheet>
    </>;
  }
  ...
  function renderMonthlyQuickForm(rowType, weekNumber) {
    ...
    return <>
      <button type="button" onClick={() => openMonthlyBlockForm(rowType, weekNumber)} className="mt-1 w-full rounded-lg border border-dashed border-slate-200 py-1 text-[9px] font-black text-slate-300 transition hover:border-sky-200 hover:text-sky-600">+ Bloque</button>
      <BottomHalfSheet open={isOpen} onClose={resetMonthlyBlockForm} title={`+ Bloque · Semana ${weekNumber}`}>
  ...
  function renderAgendaMonthlyQuickForm(rowType, weekNumber) {
    ...
    return <>
      <button type="button" onClick={() => openAgendaMonthlyBlockForm(rowType, weekNumber)} className="mt-1 w-full rounded-lg border border-dashed border-slate-200 py-1 text-[9px] font-black text-slate-300 transition hover:border-sky-200 hover:text-sky-600">+ Bloque</button>
      <BottomHalfSheet open={isOpen} onClose={resetAgendaMonthlyBlockForm} title={`+ Bloque · Semana ${weekNumber}`}>
...
function canCreateAssignments() {
  ...
  // Prioridad de carga a Semana Típica (subprocesos.prioridad_carga): solo
  // Dirección General puede definirla — decisión explícita del usuario.
  async function handleUpdateSubprocessPriority(subprocesoId, prioridad) {
    const result = await updateSubprocesoPrioridad(subprocesoId, prioridad);
    if (!result.ok) {
      window.alert("No se pudo actualizar la prioridad del subproceso.");
      return;
    }
    setSubprocesosCatalog((prev) => prev.map((sp) => (sp.id === subprocesoId ? { ...sp, prioridad_carga: prioridad } : sp)));
  }
  ...
  function validateAssignmentDraft(draft) {
    ...
    if (!draft.personaId) return "Selecciona a quién se le asigna (persona).";
  ...
  async function createAssignment() {
    ...
    // La persona a la que se le asigna es la elegida explícitamente en el
    // propio formulario ("Asignar a"), NUNCA el filtro "Filtrar por
    // persona" de arriba del módulo — antes se usaba ese filtro y una
    // asignación creada con el filtro puesto en otra persona (ej. el
    // propio usuario) terminaba guardada a su nombre en vez de a quien de
    // verdad se quería asignar, y por eso solo aparecía en su propio
    // balance.
    const assignedPerson = peopleOptions.find((person) => String(person.id) === String(assignmentDraft.personaId));
    const result = await createWorkloadAssignment({
      persona_id: assignmentDraft.personaId,
      responsable: assignedPerson?.name || selectedPersonName,
      rol: roleName,
  ...
  function renderAgendaMonthlyPlanning() {
    return (
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        <div className="grid grid-cols-[56px_repeat(4,minmax(0,1fr))] border-b border-slate-100 bg-slate-50 text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 sm:grid-cols-[90px_repeat(4,minmax(0,1fr))]">
        ...
        {agendaMonthlyMatrix.map((row) => (
          <div key={row.type} className="grid grid-cols-[56px_repeat(4,minmax(0,1fr))] border-b border-slate-100 last:border-b-0 sm:grid-cols-[90px_repeat(4,minmax(0,1fr))]" style={{ alignItems: "start" }}>
    ...
    <section className="bg-[#f4f7fb] px-6 pb-6 text-slate-950">
      <div className="mx-auto max-w-[1280px] space-y-2">
        <div className="rounded-[22px] border border-slate-200 bg-white/70 p-3 shadow-sm">
          <div className="grid grid-cols-1 gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-2 shadow-sm sm:grid-cols-2">
          ...
          <div className="mt-2 overflow-hidden rounded-[18px] border border-slate-200 bg-white shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-2 bg-[#001225] px-3 py-1.5 text-white sm:gap-3 sm:px-4">
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="text-[12px] font-black uppercase tracking-tight sm:text-[13px]">Gestión de Carga y Capacidad</h2>
                <button type="button" onClick={() => setShowWorkloadVideo(true)} className="rounded-lg bg-red-600 px-2.5 py-1 text-[9px] font-black uppercase tracking-wide text-white shadow-sm transition hover:bg-red-700 sm:px-3 sm:text-[10px]">▶ Ver video</button>
                <button type="button" onClick={() => window.open(WORKLOAD_MANUAL_URL, "_blank")} className="rounded-lg border border-white/20 bg-white px-2.5 py-1 text-[9px] font-black uppercase tracking-wide text-[#001225] shadow-sm transition hover:bg-slate-100 sm:px-3 sm:text-[10px]">📄 Manual</button>
              </div>
              <div className="flex w-full gap-1 overflow-x-auto rounded-xl bg-white/10 p-0.5 sm:w-auto"><ViewTab active={viewMode === "capacity"} onClick={() => setViewMode("capacity")}>Capacidad</ViewTab><ViewTab active={viewMode === "assignments"} onClick={() => setViewMode("assignments")}>Asignaciones</ViewTab><ViewTab active={viewMode === "pending"} onClick={() => setViewMode("pending")}>Pendientes</ViewTab><ViewTab active={viewMode === "agenda"} onClick={() => setViewMode("agenda")}>Planificación</ViewTab><ViewTab active={viewMode === "week"} onClick={() => setViewMode("week")}>Semana</ViewTab><ViewTab active={viewMode === "month"} onClick={() => setViewMode("month")}>Mes</ViewTab><ViewTab active={viewMode === "projects"} onClick={() => setViewMode("projects")}>Proyectos</ViewTab></div>
            </div>
...
{showReviewModal && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4"><div className="w-full max-w-2xl rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl"><div className="mb-3 flex items-center justify-between gap-2"><div><p className="text-xs font-black uppercase tracking-widest text-slate-900">Revisión semanal</p><p className="text-[10px] font-bold text-slate-400">Retroalimentación, firma de revisado y acciones de mejora</p></div><button type="button" onClick={() => setShowReviewModal(false)} className="flex h-7 w-7 items-center justify-center rounded-full border border-slate-200 text-sm font-black text-slate-400 hover:bg-slate-50">×</button></div><div className="mb-3 grid gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-3 md:grid-cols-[1.1fr_0.9fr_0.9fr]"><div className="rounded-xl border border-slate-200 bg-white px-3 py-2"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Plan semanal cumplido</p><div className="mt-2 flex items-end gap-2"><span className="text-3xl font-black text-[#001225]">{weeklyPlanKpi.completion}%</span><span className={`mb-1 rounded-full border px-2 py-0.5 text-[9px] font-black ${weeklyPlanKpi.hasApproval ? "border-emerald-100 bg-emerald-50 text-emerald-700" : "border-amber-100 bg-amber-50 text-amber-700"}`}>{weeklyPlanKpi.hasApproval ? "Con VOBO" : "Pendiente VOBO"}</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-emerald-500" style={{ width: `${Math.min(weeklyPlanKpi.completion, 100)}%` }} /></div></div><div className="rounded-xl border border-slate-200 bg-white px-3 py-2"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Actividades del plan</p><p className="mt-2 text-2xl font-black text-slate-900">{weeklyPlanKpi.planned}</p><p className="mt-1 text-[10px] font-bold text-slate-400">Base aprobada para la semana</p></div><div className="rounded-xl border border-slate-200 bg-white px-3 py-2"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Palomeadas</p><p className="mt-2 text-2xl font-black text-emerald-700">{weeklyPlanKpi.completed}</p><p className="mt-1 text-[10px] font-bold text-slate-400">Pendientes: {weeklyPlanKpi.pending}</p></div></div><div className="grid gap-3 md:grid-cols-2"><label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Comentarios / retroalimentación<textarea value={reviewComment} onChange={(event) => setReviewComment(event.target.value)} placeholder="Comentarios del usuario o jefe directo sobre la semana..." className="mt-1 h-28 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-semibold normal-case tracking-normal text-slate-700 outline-none" /></label><label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Acción o mejora propuesta<textarea value={improvementProposal} onChange={(event) => setImprovementProposal(event.target.value)} placeholder="Proponer acción, mejora, ajuste de carga o circunstancia relevante..." className="mt-1 h-28 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-semibold normal-case tracking-normal text-slate-700 outline-none" /></label></div><div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-100 bg-slate-50 p-2"><span className={`rounded-full border px-3 py-1 text-center text-[10px] font-black ${reviewStatus.status === "Revisada" ? "border-emerald-100 bg-emerald-50 text-emerald-700" : "border-slate-200 bg-white text-slate-400"}`}>{reviewStatus.status === "Revisada" ? `Revisada por ${reviewStatus.reviewedBy} · ${reviewStatus.reviewedRole} · ${reviewStatus.reviewedAt}` : reviewStatus.status}</span><div className="flex gap-2"><button type="button" disabled={!canReviewPlan()} onClick={markPlanReviewed} className={`rounded-lg px-3 py-1.5 text-[10px] font-black ${canReviewPlan() ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-300 cursor-not-allowed"}`}>Firmar revisado</button><button type="button" onClick={() => setShowReviewModal(false)} className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-[10px] font-black text-slate-500">Cerrar</button></div></div></div></div>}{agendaView === "weekly" && <div className="grid grid-cols-5 gap-1 sm:gap-2">{agendaWeekSummary.map((day) => { const overflow = day.occupation > 100; return <div key={day.day} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); reorderAgendaBlock(agendaDraggedBlock, day.day, day.activities.length); }} className={`min-w-0 rounded-2xl border bg-white shadow-sm transition ${agendaDraggedBlock ? "border-sky-200 bg-sky-50/30" : "border-slate-200"}`}><div className="border-b border-slate-100 px-2.5 py-2"><div className="flex items-center justify-between gap-2"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-600">{day.day}</p><StatusPill status={day.status} /></div><div className="mt-1.5 flex items-center justify-between text-[9px] font-black text-slate-500"><span>{formatHours(day.usedMinutes / 60)} / {formatHours(day.capacityHours)}</span><span className={overflow ? "text-red-500" : "text-slate-700"}>{day.occupation.toFixed(0)}%</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full ${day.status.bar}`} style={{ width: `${Math.min(day.occupation, 140)}%` }} /></div></div><div className="space-y-1 p-2">{day.activities.map((activity, activityIndex) => <React.Fragment key={activity.occurrenceId || activity.id}>{renderAgendaInsertLine(day.day, activityIndex)}<div draggable onDragStart={() => setAgendaDraggedBlock(activity)} onDragEnd={() => { setAgendaDraggedBlock(null); setAgendaDropIndicator(null); }} title={`${activity.actividad}: ${formatMinutesAsTime(activity.horaInicioMinutos)} – ${formatMinutesAsTime(activity.horaFinMinutos)}`} className={`relative cursor-grab rounded-lg border px-1.5 py-1 shadow-sm transition active:cursor-grabbing ${agendaDraggedBlock && getAgendaBlockKey(agendaDraggedBlock) === getAgendaBlockKey(activity) ? "opacity-40" : ""} ${getCardStyle(activity.origen)}`}><SemaforoDot value={activity.semaforo} onChange={(next) => handleSemaforoChange(activity.sourceActivityId || activity.id, next)} disabled={!canEditSelectedPerson} className="z-20" /><button type="button" onClick={() => toggleAgendaBlockCompleted(activity)} className={`absolute left-1 top-1 flex h-4 w-4 items-center justify-center rounded-full text-[9px] font-black transition ${completedAgendaBlockIds.includes(getAgendaBlockKey(activity)) ? "bg-emerald-500 text-white" : "text-slate-300 hover:text-emerald-500"}`} title="Marcar realizado">✓</button><select value="" onMouseDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()} onChange={(event) => { handleAgendaScheduledAction(activity, event.target.value); event.target.value = ""; }} className="absolute right-1 top-1 z-20 h-4 w-6 rounded-full border border-slate-200 bg-white text-[9px] font-black text-slate-500 outline-none"><option value="">⋯</option><option value="up">Subir</option><option value="down">Bajar</option><option value="move">Mover</option><option value="edit">Editar</option>{canRemoveAgendaBlock(activity) && <option value="remove">Quitar</option>}</select><div className="flex items-start gap-1 px-5">{showScheduleColumn && <div className="w-6 shrink-0 border-r border-black/10 pr-0.5 text-center text-[6px] font-black leading-none tracking-tighter text-slate-500">{formatMinutesAsTime(activity.horaInicioMinutos)}</div>}<div className="min-w-0 flex-1"><p className="line-clamp-2 text-[9px] font-black leading-tight text-slate-950">{activity.actividad}</p><p className="mt-0.5 truncate text-[8px] font-bold text-slate-500">{activity.planningSource || "Bloque adicional"}</p>{getPendingActivitySubprocess(activity) && <p className="mt-0.5 truncate text-[7px] font-bold text-slate-400">{getPendingActivitySubprocess(activity)}</p>}<div className="mt-1 flex items-end justify-between gap-2"><span className={`inline-flex rounded-md border px-1.5 py-0.5 text-[7px] font-black uppercase tracking-wide ${getCardAccentStyle(activity.origen)}`}>{activity.origen}</span><span className="text-[8px] font-black text-slate-500">{activity.duracionMinutos} min</span></div></div></div></div></React.Fragment>)}{renderAgendaInsertLine(day.day, day.activities.length)}<button type="button" onClick={() => setAgendaQuickDay(day.day)} className="w-full rounded-lg border border-dashed border-slate-200 py-1 text-[9px] font-black text-slate-300 transition hover:border-sky-200 hover:text-sky-600">+ Bloque</button><BottomHalfSheet open={agendaQuickDay === day.day} onClose={resetAgendaBlockForm} title={`+ Bloque · ${day.day}`}><div className="rounded-lg border border-sky-100 bg-sky-50/40 p-1.5"><select value={agendaBlockType} onChange={(event) => setAgendaBlockType(event.target.value)} className="mb-1 w-full rounded-md border border-slate-200 bg-white px-2 py-1 text-[9px] font-bold text-slate-700 outline-none"><option value="Proyecto">Proyecto</option><option value="Formación">Formación</option><option value="Tarea">Tarea</option><option value="Eventual">Eventual</option></select><input value={agendaBlockName} onChange={(event) => setAgendaBlockName(event.target.value)} placeholder="Nombre" className="mb-1 w-full rounded-md border border-slate-200 bg-white px-2 py-1 text-[9px] font-bold text-slate-700 outline-none" /><div className="flex gap-1"><input value={agendaBlockMinutes} onChange={(event) => setAgendaBlockMinutes(event.target.value)} type="number" min="1" placeholder="Min" className="min-w-0 flex-1 rounded-md border border-slate-200 bg-white px-2 py-1 text-[9px] font-bold text-slate-700 outline-none" /><button type="button" onClick={() => saveAgendaBlock(day.day)} className="rounded-md bg-[#001225] px-2 py-1 text-[8px] font-black text-white">OK</button><button type="button" onClick={resetAgendaBlockForm} className="rounded-md border border-slate-200 bg-white px-2 py-1 text-[8px] font-black text-slate-400">X</button></div></div></BottomHalfSheet></div></div>; })}</div>}{agendaView === "monthly" && renderAgendaMonthlyPlanning()}{false && <div className="grid grid-cols-4 gap-2">{agendaMonthSummary.map((week) => <div key={week.weekNumber} className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm"><div className="mb-2 flex items-center justify-between gap-2"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-600">{week.label}</p><StatusPill status={week.status} /></div><div className="mb-2 flex items-center justify-between text-[9px] font-black text-slate-500"><span>{formatHours(week.usedMinutes / 60)} / {formatHours(week.capacityHours)}</span><span>{week.occupation.toFixed(0)}%</span></div><div className="mb-3 h-1.5 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full ${week.status.bar}`} style={{ width: `${Math.min(week.occupation, 140)}%` }} /></div><div className="space-y-1">{MONTH_MATRIX_ROWS.map((rowType) => { const sourceType = rowType === "Procesos" ? "Proceso" : rowType === "Proyectos" ? "Proyecto" : "Formación"; const count = activeAgendaBlocks.filter((activity) => activity.origen === sourceType).length; const minutes = activeAgendaBlocks.filter((activity) => activity.origen === sourceType).reduce((sum, activity) => sum + getDurationMinutes(activity), 0); return <div key={rowType} className="rounded-lg border border-slate-100 bg-slate-50/60 px-2 py-1"><div className="flex items-center justify-between gap-2"><SourcePill source={rowType} /><span className="text-[9px] font-black text-slate-500">{formatHours(minutes / 60)}</span></div><p className="mt-0.5 text-[8px] font-bold text-slate-400">{count} bloques planificados</p></div>; })}</div></div>)}</div>}</div>}
            ...
            {viewMode === "projects" && (
              <div className="p-3">
                {pmoRecordatorios.length > 0 && (
                  <div className="mb-3 space-y-1.5">
                    {pmoRecordatorios.map((r) => (
                      <div key={r.id} className="flex items-center justify-between gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-[10px] font-bold text-amber-800">
                        <span>🔔 <strong>{r.proyecto?.nombre || "Proyecto"}:</strong> {r.mensaje}</span>
                        <button type="button" onClick={() => handleDismissPmoRecordatorio(r.id)} className="shrink-0 rounded-md border border-amber-300 bg-white px-2 py-0.5 text-[9px] font-black text-amber-700 hover:bg-amber-100">Visto</button>
                      </div>
                    ))}
                  </div>
                )}
                    ...
                    <table className="w-full table-fixed text-[10px]">
                      <thead>
                        <tr className="border-b border-slate-200 bg-slate-100 text-slate-600">
                          <th className="w-[13%] px-2.5 py-2 text-left font-black tracking-wide">Proyecto</th>
                          <th className="w-[12%] px-2.5 py-2 text-left font-black tracking-wide">Líder proyecto</th>
                          <th className="w-[12%] px-2.5 py-2 text-left font-black tracking-wide">Líder proceso</th>
                          <th className="w-[8%] px-2.5 py-2 text-left font-black tracking-wide">Etapa</th>
                          ...
                          <th className="w-[9%] px-2.5 py-2 text-left font-black tracking-wide">Próx. hito</th>
                          <th className="w-[9%] px-2.5 py-2 text-left font-black tracking-wide">Fecha</th>
                          <th className="w-[13%] px-2.5 py-2 text-left font-black tracking-wide">Decisión</th>
                              ...
                              <tr className={`border-b border-slate-100 border-l-4 ${semaforoAccent} ${zebra} align-top transition hover:bg-sky-50/40`}>
                                ...
                                <td className="px-2.5 py-2">
                                  <select
                                    ...
                                    disabled={!canEdit}
                                    title={peopleOptions.find((p) => String(p.id) === String(proyecto.lider_proyecto_persona_id))?.name || "Sin asignar"}
                                ...
                                <td className="px-2.5 py-2">
                                  <select
                                    ...
                                    disabled={!canEdit}
                                    title={proyecto.lider_proceso || "Sin asignar"}
                                ...
                                <tr className="border-b border-slate-100 bg-amber-50/50">
                                  <td colSpan={10} className="px-3 py-2">
                                    <div className="flex items-center gap-2">
                                      <input
                                        type="text"
                                        value={pmoRecordatorioText}
                                        onChange={(e) => setPmoRecordatorioText(e.target.value)}
                                        placeholder={`Mensaje para ${proyecto.lider_proyecto?.nombre || "el líder"}...`}
                                        className="h-8 flex-1 rounded-lg border border-slate-200 bg-white px-2 text-[11px] font-bold text-slate-700 outline-none"
                                      />
