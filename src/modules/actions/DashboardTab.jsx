...
function StatCard({ label, value, color }) {
  return (
    <div className="rounded-2xl border p-3 shadow-sm" style={{ borderColor: `${color}3d`, background: `linear-gradient(180deg, ${color}12 0%, #ffffff 70%)` }}>
...
export default function DashboardTab({ acciones, procesosById }) {
  ...
  return (
    <div className="space-y-3">
      <div className="grid gap-3 grid-cols-2 md:grid-cols-4">
        <StatCard label="Abiertas" value={abiertas} color="#2a78d6" />
        <StatCard label="Cerradas" value={cerradas} color="#0ca30c" />
        <StatCard label="Con riesgo" value={conRiesgo} color="#d03b3b" />
        <StatCard label="Vencidas" value={vencidas} color="#eda100" />
      </div>
